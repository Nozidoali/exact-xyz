...
void cardinality_step(const QRState& state, std::vector<std::shared_ptr<QGate>>& gates, QRState& new_state);
...
std::pair<uint32_t, uint32_t> maximize_difference_once(uint32_t num_qubits, std::vector<uint32_t>& indices,
                                                       std::map<uint32_t, bool>& diff_values) {
    int      max_diff       = -1;
    uint32_t max_diff_qubit = 0;
    bool     max_diff_value = false;
    ...
    for (uint32_t qubit = 0; qubit < num_qubits; qubit++) {
        ...
        for (uint32_t index : indices)
            if ((index >> qubit) & 1)
                indices_1.push_back(index);

        int diff = std::abs((int)length - 2 * (int)indices_1.size());
        if (diff == (int)length)
            continue;
        if (diff > max_diff) {
            max_diff       = diff;
            ...
            if (max_diff_value) {
                ...
                for (uint32_t idx : indices)
                    if (std::find(indices_1.begin(), indices_1.end(), idx) == indices_1.end())
...
void cardinality_step(const QRState& state, std::vector<std::shared_ptr<QGate>>& gates, QRState& new_state) {
    ...
    for (const auto& [index, weight] : state.index_to_weight) {
        if (index == index0)
            continue;
        bool valid = true;
        for (const auto& [qubit, value] : diff_values)
            if (((index >> qubit) & 1) != (uint32_t)value) {
                valid = false;
                break;
            }
        if (valid)
    ...
    while (candidates.size() > 1) {
        std::vector<uint32_t> cands_copy = candidates;
        maximize_difference_once(state.n_bits, cands_copy, diff_values);
        candidates = cands_copy;
    }

    uint32_t index1 = candidates[0];
    for (uint32_t qubit = 0; qubit < state.n_bits; qubit++) {
        if (((index0 >> qubit) & 1) == ((index1 >> qubit) & 1))
            continue;
        if (qubit == diff_qubit)
            continue;
        gates.push_back(std::make_shared<CX>(diff_qubit, diff_value, qubit));
    ...
    uint32_t idx0  = index1 & (~(1 << diff_qubit));
    uint32_t idx1  = index1 | (1 << diff_qubit);
    double   w0    = new_state.index_to_weight[idx0];
    double   w1    = new_state.index_to_weight[idx1];
    double   theta = 2.0 * std::atan2(w1, w0);
    if (index1 & (1 << diff_qubit))
        theta = -M_PI + theta;

    gates.push_back(std::make_shared<MCRY>(ctrls, phases, theta, diff_qubit));

...
Complex tr(const Matrix2x2& a) { return a[0][0] + a[1][1]; }
...
Matrix2x2 normalize_to_su2(const Matrix2x2& u) {
    Complex d = det2(u);
    if (std::abs(d) < 1e-15) return u;
    Complex s = std::sqrt(d);
    if (std::abs(s) < 1e-15) return u;
    Matrix2x2 r = u;
    r[0][0] /= s; r[0][1] /= s;
    r[1][0] /= s; r[1][1] /= s;
    return r;
...
Matrix2x2 mat_X() { return Matrix2x2{{ {Complex(0,0), Complex(1,0)}, {Complex(1,0), Complex(0,0)} }}; }
Matrix2x2 mat_Y() { return Matrix2x2{{ {Complex(0,0), Complex(0,-1)}, {Complex(0,1), Complex(0,0)} }}; }
Matrix2x2 mat_Z() { return Matrix2x2{{ {Complex(1,0), Complex(0,0)}, {Complex(0,0), Complex(-1,0)} }}; }
...
void su2_axis_angle(const Matrix2x2& U_in, double& nx, double& ny, double& nz, double& phi) {
    ...
    if (nrm < 1e-12) { nx=0; ny=0; nz=1; }
    else { nx/=nrm; ny/=nrm; nz/=nrm; }
}
...
Matrix2x2 word_to_matrix(const Word& w) {
    Matrix2x2 U = mat_I();
    for (char g : w) {
        if (g == 'H') U = mmul(U, mat_H());
        else if (g == 'T') U = mmul(U, mat_T());
        else if (g == 't') U = mmul(U, mat_Tdg());
        else throw std::runtime_error("Unknown gate symbol in word");
    }
    return U;
}
...
Word invert_word(const Word& w) {
    ...
    for (auto it = w.rbegin(); it != w.rend(); ++it) {
        char g = *it;
        if (g == 'H') inv.push_back('H');
        else if (g == 'T') inv.push_back('t');
        else if (g == 't') inv.push_back('T');
        else throw std::runtime_error("Unknown gate symbol in word");
    }
    return inv;
...
std::string hash_upto_phase(const Matrix2x2& U_in) {
    Matrix2x2 U = normalize_to_su2(U_in);
    Complex z = U[0][0];
    if (std::abs(z) < 1e-12) z = U[1][1];
    if (std::abs(z) > 1e-12) {
        Complex ph = z / std::abs(z);
        Complex s = std::conj(ph);
        for (int r=0;r<2;r++) for (int c=0;c<2;c++) U[r][c] *= s;
    }
...
struct NetElem { Matrix2x2 U; Word w; };
...
std::vector<NetElem> build_net(int max_len) {
    ...
    while (!q.empty()) {
        NetElem cur = q.front(); q.pop();
        if ((int)cur.w.size() == max_len) continue;
        for (auto& [sym, G] : gens) {
            NetElem nxt;
            nxt.U = mmul(cur.U, G);
            nxt.w = cur.w;
...
std::vector<std::shared_ptr<QGate>> approximate_ry_sk(uint32_t target, double theta, double eps) {
    ...
    for (int depth = 0; depth <= MAX_DEPTH; depth++) {
        ...
        if (d < best_d) { best_d = d; best_word = std::move(w); }
        if (d <= eps) break;
    }
    ...
    for (char g : best_word) {
        if (g == 'H') gates.push_back(std::make_shared<H>(target));
        else if (g == 'T') gates.push_back(std::make_shared<T>(target));
        else if (g == 't') gates.push_back(std::make_shared<Tdg>(target));
        else throw std::runtime_error("approximate_ry_sk: unknown symbol in synthesized word");
    }
    return gates;
}

...
void enumerate_cnot_templates(std::vector<CXT>& templates, const std::vector<uint32_t>& controls, uint32_t n_cnots,
                              uint32_t n_cnots_added, CXT& curr_template) {
...
std::optional<std::pair<RSOL, CXT>> rotation_solver(const RLUT& rlut, const std::vector<uint32_t>& controls,
                                                    uint32_t max_cnots) {
...
void resyn_impl(const QCircuit& circuit, QCircuit& new_circuit, const QRState& initial_state, uint32_t pos) {
    ...
    auto sol = rotation_solver(rotation_lut, controls, initial_cost);
    if (sol) {
    ...
    resyn_impl(circuit, new_circuit, new_state, new_pos);
}
} // namespace detail

QCircuit resyn(const QCircuit& circuit, bool verbose) {
    QCircuit new_circuit(circuit.num_qbits);
    (void)verbose;
    detail::resyn_impl(circuit, new_circuit, ground_rstate(circuit.num_qbits), 0);
    return new_circuit;
}

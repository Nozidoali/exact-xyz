...
TEST( QGateTest, ApplyCRy1 )
{
  ...
  QRState state( index_to_weight, 3 );
  CRY gate( 1, 0, M_PI / 2, 0 );
  QRState new_state = gate( state );
  EXPECT_LT( std::abs( new_state.index_to_weight[0b011] - 1 ), QRState::eps );
  EXPECT_LT( std::abs( new_state.index_to_weight[0b001] - sqrt( 2 ) ), QRState::eps );
...
TEST( QGateTest, ApplyCRy2 )
{
  ...
  QRState new_state = gate1( state );
  EXPECT_LT( std::abs( new_state.index_to_weight[0b011] - 1 ), QRState::eps );
  EXPECT_LT( std::abs( new_state.index_to_weight[0b000] - sqrt( 5 ) ), QRState::eps );
  ...
  EXPECT_LT( std::abs( new_state.index_to_weight[0b011] - 1 ), QRState::eps );
  EXPECT_LT( std::abs( new_state.index_to_weight[0b001] - sqrt( 5 ) ), QRState::eps );
  EXPECT_EQ( new_state.index_to_weight.find( 0b000 ), new_state.index_to_weight.end() );
}
